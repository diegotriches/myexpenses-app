...
export function CardCategoriasLancamentos({ transacoes }: Props) {
  ...
  const totalLancamentos = categorias.reduce((acc, c) => acc + Number(c.total), 0);
        ...
        {categorias.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {categorias.map((c, index) => {
              const porcentagem = totalLancamentos > 0 ? (Number(c.total) / totalLancamentos) * 100 : 0;
            ...
            <div className="pt-2 border-t border-gray-200 flex justify-between items-center">
              <span className="text-sm font-medium text-gray-600">Total em lançamentos:</span>
